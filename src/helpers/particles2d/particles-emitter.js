...
export class ParticlesEmitter {
    ...
    spawn() {
        ...
        this.updateTween = tweens.dummy({ time: 1000, repeat: Infinity });
        this.updateTween.onUpdate(() => this.update());
    }
    ...
    emitMultiple(amount, delay = 100, position, key) {
        for (let i = 0; i < amount; i++) {
            setTimeout(() => this.emit(position, key), utils2d.randomInt(0, delay));
        }
    }

    emitByInterval(amount = 1, interval = 100, position, key) {
        const handler = () => this.emitMultiple(amount, 0, position, key);
    ...
    kill() {
        this.enabled = false;
        this.pool.forEach((particle) => {
            particle.kill(this.particleData.hideTween);
        });
    }
}
